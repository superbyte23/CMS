...
class Participant extends DB{
	...
	public function participants_in_program_id($id){  
		$sql = "SELECT * FROM `participants` LEFT JOIN department ON participants.department = department.department_id WHERE program_id = :id";
		$stmt = $this->conn->prepare($sql);
	...
	public function participants_by_id($id){  
		$sql = "SELECT * FROM `participants` WHERE participant_id = :id";
		$stmt = $this->conn->prepare($sql);
		$stmt->bindValue(':id', $id);
		$stmt->execute();
	...
	protected static $tbl_name = "participants";
